loss_ic <- function(y_true, y_pred) {
  ...
  gLl <- tf$exp(output_pred) * output_left
  gLr <- tf$exp(output_pred) * output_right
  status <- tf$reshape(status, tf$shape(gLl))
  status <- tf$cast(status, tf$float32)
  lik_ic = tf$exp(-1 * gLl) -  status * tf$exp(-1 * gLr)

  lik_ic_nozeros = tf$boolean_mask(tensor = lik_ic, mask = tf$greater(lik_ic, 0))
  loglik_ic = tf$reduce_sum(tf$math$log(lik_ic_nozeros))
...
bern <- function(j,m,l,u,t){
  ...
  BS = (tf$exp(tf$math$lgamma( tf$cast(m_new + tf$ones_like(m_new), dtype = tf$float32) ))/(tf$exp(tf$math$lgamma( tf$cast(j_new + tf$ones_like(m_new), dtype = tf$float32) ))*tf$exp(tf$math$lgamma(  tf$cast(m_new - j_new + tf$ones_like(m_new), dtype = tf$float32) ))))*(tf$pow((t-l)/(u-l), j_new))*(tf$pow( tf$ones_like(m_new) -(t-l)/(u-l), m_new-j_new))
  return(BS)
}
...
SemiParLayer <- R6::R6Class("SemiParLayer",
                            ...
                            public = list(
                              ...
                              initialize = function(output_dim, m, l, u) {
                                ...
                                self$l <- l
                                self$u <- u
                                self$m_new <- tf$cast(m + 1, dtype = tf$int32)
                              },
                              ...
                              call = function(time, mask = NULL) {
                                ...
                                bp = bp$stack()
                                bp = tf$transpose(tf$reshape(bp, c(tf$shape(bp)[1], -1L)))

                                ep <- tf$math$cumsum(tf$exp(self$phi), axis = 0L)
                                gL <- tf$matmul(bp, tf$transpose(ep))
                                gL
...
layer_semipar <- function(object, output_dim, m, l, u, name = NULL, trainable = TRUE) {
  ...
  # Apply metadata after creation (works across versions)
  if (!is.null(name)) layer$name <- name
...
build_model_ic <- function(left, right, pred, m, l = 0, u = NULL, num_nodes, string_activation, num_l1, num_dropout, num_lr, num_layer) {
  ...
  model %>% compile(
    optimizer = optimizer_rmsprop(learning_rate = num_lr),
    loss = loss_ic,
    metrics = NULL
  )
...
brier_ic_nn <- function(obj, left_new, right_new, x_new, btime = NULL, type = c("IBS", "BS")) {
  ...
  if (is.null(btime)) btime=range(intL, intR)
    ...
    S_times = t(k_get_value(exp(-1 * (tf$matmul(tf$cast(matrix(exp(out.1[,3]), ncol = 1), dtype = tf$float32), tf$cast(matrix(out.2[,1], nrow = 1), dtype = tf$float32))))))
